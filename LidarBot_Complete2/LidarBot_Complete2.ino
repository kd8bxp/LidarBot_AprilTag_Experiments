...
void findTag() {
  ...
  if (int(r) <= 341 && int(r) >= 291) { M5.Lcd.println("Bottom Left"); move(turnRight); delay(150); move(stop); return; }//robot is facing Bottom Left ? currentTag = tagInfo.id - 6 ?
  if (int(r) <= 251 && int(r) >= 201) { M5.Lcd.println("Bottom Right"); move(turnLeft); delay(150); move(stop); return; } //robot is facing bottom right? currentTag = tagInfo.id - 8 ?
  if (int(r) <= 161 && int(r) >= 111) { M5.Lcd.println("Top-Right"); move(turnRight); delay(150); move(stop); return; } //robot is facing top right? currentTag = tagInfo.id + 6 ?
  if (int(r) <= 71 && int(r) >= 21) { M5.Lcd.println("Top-Left"); move(turnLeft); delay(150); move(stop); return;  } //robot is facing top left? currentTag = tagInfo.id + 8 ?
